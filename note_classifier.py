...
class NoteClassifier:
    """
    Классификатор типов нот и их позиций на нотоносце
    """
    
    # Правильный маппинг для скрипичного ключа
    TREBLE_CLEF = {
        -4: ('A', 'Ля'),
        -3: ('B', 'Си'),
        -2: ('C', 'До'),
    ...
    def determine_position(self, note, staff_lines):
        """
        Определяет положение ноты на нотоносце
        Использует центр ноты для точного определения
        """
        if staff_lines is None or len(staff_lines) == 0:
            return 4
        
        # Используем Y центр ноты
        ...
        # Находим 5 ближайших линий к ноте
